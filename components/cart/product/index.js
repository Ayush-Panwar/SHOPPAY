...
export default function Product({ product, selected, setSelected }) {
  ...
  const updateQty = (type) => {
    let newCart = cart.cartItems.map((p) => {
      if (p._uid == product._uid) {
        selected.map((s) => {
          if (s._uid == product._uid) {
            setSelected([
              ...selected.filter((select) => select._uid != product._uid),
              { ...s, qty: type == "plus" ? product.qty + 1 : product.qty - 1 },
            ]);
          }
        });
        return {
  ...
  const removeProduct = (id) => {
    ...
    setSelected([...selected.filter((p) => p._uid != id)]);
    dispatch(updateCart(newCart));
  };
  const handleSelect = () => {
    if (active) {
